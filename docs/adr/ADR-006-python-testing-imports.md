...
# From lambda/tests/unit/, parents[2] is lambda/, then into the function dir
lambda_dir = Path(__file__).resolve().parents[2] / "function-name"
...
handler_path = Path(__file__).parents[2] / "function-name" / "handler.py"
...
# Add Lambda function to Python path
lambda_dir = Path(__file__).resolve().parents[2] / "query-kb"
sys.path.insert(0, str(lambda_dir))
