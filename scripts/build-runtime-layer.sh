...
# Build a Lambda layer containing git, make, and runtime tools
...
# Also clean java/ in case it exists from old builds when Java was in this layer
sudo rm -rf "$LAYER_DIR"/{bin,lib,libexec,java} 2>/dev/null || rm -rf "$LAYER_DIR"/{bin,lib,libexec,java} 2>/dev/null || true
...
This layer provides git, make, and related build tools for the run-tests Lambda.

**Note:** Layer binaries (`bin/`, `lib/`) are not committed to git. You must build the layer locally before running `terraform apply`.
