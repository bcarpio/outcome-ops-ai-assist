...
def handler(event, context):
    ...
    try:
        ...
        # Filter repos based on event payload (if specified)
        repos_to_process = allowlist["repos"]
        if "repos" in event and event["repos"]:
            requested_repos = set(event["repos"])
            repos_to_process = [
                repo for repo in allowlist["repos"]
                if repo["name"] in requested_repos
            ]
            logger.info(f"Filtering to requested repos: {requested_repos}")
            if not repos_to_process:
                logger.warning(f"No matching repos found in allowlist for: {requested_repos}")
        ...
        for repo_config in repos_to_process:
            ...
            try:
                ...
                # Successfully processed this repo (even if some docs failed)
                repos_processed += 1
        ...
        logger.info(f"Document ingestion completed: {repos_processed} repos, {total_docs_ingested} documents")
